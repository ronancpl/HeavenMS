...
function start() {
    ...
    if (!Packages.constants.YamlConfig.config.server.USE_CPQ) {
        ...
        if (cm.getPlayer().getMapId() == 980000010) {
            ...
            if (status == 0) {
                ...
                if (cm.fieldTaken(selection)) {
                    ...
                    var party = cm.getParty().getMembers();
                    if ((selection >= 0 && selection <= 3) && party.size() < (Packages.constants.YamlConfig.config.server.USE_ENABLE_SOLO_EXPEDITIONS ? 1 : 2)) {
                        cm.sendOk("You need at least 2 players to participate in the battle!");
                    } else if ((selection >= 4 && selection <= 5) && party.size() < (Packages.constants.YamlConfig.config.server.USE_ENABLE_SOLO_EXPEDITIONS ? 1 : 3)) {
