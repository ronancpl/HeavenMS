    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (cm.fieldTaken2(selection)) {
                ...
                var party = cm.getParty().getMembers();
                if ((selection === 0 || selection === 1 ) && party.size() < (Packages.constants.YamlConfig.config.server.USE_ENABLE_SOLO_EXPEDITIONS ? 1 : 2)) {
                    cm.sendOk("You need at least 2 players to participate in the battle!");
                } else if ((selection === 2 ) && party.size() < (Packages.constants.YamlConfig.config.server.USE_ENABLE_SOLO_EXPEDITIONS ? 1 : 3)) {
